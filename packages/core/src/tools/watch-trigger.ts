...
export default class WatchTriggerTool implements Tool {
  ...
  async execute(
    ...
    while (new Date().getTime() - start < timeout) {
      ...
      let changed = await this.has_openbrowser_changed(agentContext);
      if (changed == "false") {
        continue;
      }
      await this.init_openbrowser_observer(agentContext);
    ...
    try {
      ...
      await screenshot.call(
        ...
        () => {
          ...
          openbrowser_observer.observe(document.body, {
            childList: true,
            subtree: true,
            attributes: true,
      ...
      console.error("Error initializing OpenBrowser observer:", error);
    }
  }

  private async has_openbrowser_changed(
    agentContext: AgentContext
    ...
    try {
      ...
      let result = (await screenshot.call(
        ...
        agentContext,
        () => {
          return (window as any).has_openbrowser_changed + "";
        },
        []
      )) as string;
      return result as any;
    } catch (e) {
      console.error("Error checking OpenBrowser change:", e);
      return "undefined";
    }
  }
