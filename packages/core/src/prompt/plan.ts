...
## Example 5
User: Search for information about "openbrowser" compile the results into a summary profile, then share it across social media platforms including Twitter, Facebook, and Reddit. Finally, export the platform sharing operation results to an Excel file.
Output result:
<root>
<name>OpenBrowser Research and Social Media Campaign</name>
<thought>The user wants me to research information about 'OpenBrowser', create a summary profile, share it on multiple social media platforms (Twitter, Facebook, Reddit), and then compile the results into an Excel file. This requires multiple agents working together: Browser for research, Browser for social media posting (Twitter, Facebook, and Reddit in parallel), and File for creating the Excel export. I need to break this down into sequential steps with proper variable passing between agents.</thought>
<agents>
  <agent name="Browser" id="0" dependsOn="">
      <task>Research comprehensive information about 'OpenBrowser'</task>
      <nodes>
        <node>Search for the latest information about 'OpenBrowser' - its identity, purpose, and core features</node>
        <node>Search for OpenBrowser's functionalities, capabilities, and technical specifications</node>
        <node>Search for recent news, updates, announcements, and developments related to OpenBrowser</node>
        <node>Search for user reviews, feedback, and community discussions about OpenBrowser</node>
        <node>Search for OpenBrowser's market position, competitors, and industry context</node>
    ...
    <agent name="Browser" id="1" dependsOn="0">
      <task>Share OpenBrowser's summary and collected interaction data on Twitter/X</task>
      <nodes>
        <node>Navigate to Twitter/X platform</node>
        <node input="researchData">Create and post Twitter-optimized content about OpenBrowser (within character limits, using hashtags)</node>
    ...
    <agent name="Browser" id="2" dependsOn="0">
      <task>Share OpenBrowser's summary and collected interaction data on Facebook</task>
      <nodes>
        <node>Navigate to Facebook platform</node>
        <node input="researchData">Create and post Facebook-optimized content about OpenBrowser (longer format, engaging description)</node>
    ...
    <agent name="Browser" id="3" dependsOn="0">
      <task>Share OpenBrowser's summary and collected interaction data on Reddit</task>
      <nodes>
        <node>Navigate to Reddit platform</node>
        <node input="researchData">Find appropriate subreddit and create Reddit-optimized post about OpenBrowser (community-focused, informative)</node>
    ...
    <agent name="File" id="4" dependsOn="1,2,3">
      ...
      <nodes>
        ...
        <node>Save the file as 'OpenBrowser_Social_Media_Campaign_Results.xlsx'</node>
      </nodes>
    </agent>
  </agents>
</agents>
